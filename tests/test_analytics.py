...
def collector(tmp_path: Path) -> AnalyticsCollector:
    return AnalyticsCollector(
        events_path=tmp_path / "events.jsonl", failures_path=tmp_path / "failures.jsonl"
    )
...
def test_log_comparison_and_indexing_and_note(collector: AnalyticsCollector):
    collector.log_comparison(
        ["p1", "p2"], generation_time=1.2, result_length=500, aspects_count=3
    )
    collector.log_indexing(
        "p1", chunk_count=8, embedding_time=0.3, parse_time=0.05, persist_time=0.02
    )
...
def test_log_failure_writes_to_failures_file(collector: AnalyticsCollector):
    collector.log_failure(
        "retrieval", {"paper_id": "p1", "query": "q"}, reason="low score"
    )
...
def _write_retrieval_report(path: Path, results: list[dict]) -> None:
    path.write_text(
        json.dumps(
            {"dataset": "x", "summary": {}, "results": results}, ensure_ascii=False
        ),
        encoding="utf-8",
    )
    ...
    results = [
        {
            ...
            "mrr": 1.0,
            "retrieved_chunks": [
                {"is_relevant": True, "section": "A", "paper_id": "p1"}
            ],
        },
        {
            "sample_id": "2",
        ...
        },
        {
            ...
            "mrr": 0.0,
            "retrieved_chunks": [
                {"is_relevant": False, "section": "B", "paper_id": "p2"}
            ],
        },
    ]
...
def test_plot_failure_case_heatmap():
    fig = visualizer.plot_failure_case_heatmap(
        {"p1": {"Method": 2}, "p2": {"Results": 3}}
    )
    assert fig.axes

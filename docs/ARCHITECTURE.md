...
ResearchAgent implements the Model Context Protocol (MCP) as an agent-client pattern for external tool integration.

### MCP Components

```
ResearchAgent (MCP Agent)
├─ MCPClientManager (app/mcp/client_manager.py)
│  ├─ Manages MCP server process lifecycle
│  ├─ Thread-safe server registry
│  └─ Auto-restart on failure
│
├─ MCPToolProxy (app/mcp/tool_proxy.py)
│  ├─ Unified interface for MCP tool calls
│  └─ Handles protocol communication (stub)
│
└─ MCP Adapters (app/research_workflow/*_mcp_adapter.py)
   ├─ ZoteroMCPAdapter: Collection management
   ├─ SemanticScholarMCPAdapter: Paper search
   └─ ArxivMCPAdapter: Preprint search
```

### MCP Data Flow

```
ResearchRunService.__init__()
    ↓
_init_mcp_manager()
    ↓
MCPClientManager.start_server("zotero")
    ├─ ensure_zotero_mcp_installed()
    ├─ subprocess.Popen(["zotero-mcp"], env=...)
    └─ Register MCPServerProcess
    ↓
(Future) MCPToolProxy.call_tool(MCPToolCall)
    ├─ Get client from manager
    ├─ Invoke tool via MCP protocol
    └─ Return MCPToolResult
    ↓
ZoteroMCPAdapter.list_collection_items(collection_id)
    ├─ Build MCPToolCall
    ├─ Call proxy
    └─ Parse result to domain models
```
...
- `mcp_startup_timeout`: Server startup timeout (default: 10.0s)
- `mcp_health_check_interval`: Health check frequency (default: 30.0s)
- `mcp_tool_timeout`: Tool call timeout (default: 30.0s)
- `zotero_mcp_enabled`: Zotero integration (default: True)
- `zotero_mcp_auto_install`: Auto-install server (default: True)
- `zotero_data_dir`: Zotero data directory path

### Thread Safety

MCPClientManager uses `threading.Lock()` for concurrent access:

```python
with self._lock:
    # Atomic operations on _servers dict
    self._servers[name] = server
```

Server lifecycle operations (terminate, wait, kill) happen **outside** the lock to prevent deadlock during blocking I/O.

### Error Handling

- **Startup failure**: Logged as warning, doesn't crash app
- **Tool call error**: Returned as MCPToolResult with error field
- **Server crash**: Auto-restart if `auto_restart=True`

### Future Implementation

Current implementation is stub-quality for demo. Full implementation requires:

1. **MCP Protocol Communication**: Integrate `mcp.client` SDK for stdio transport
2. **Tool Discovery**: List available tools from MCP servers
3. **Health Monitoring**: Periodic health checks with auto-restart
4. **Retry Logic**: Exponential backoff for failed tool calls
5. **Telemetry**: Track tool usage, latency, error rates

...
class HybridRetriever:
    ...
    def search(self, query: str, top_k: int = 5, paper_id: str | None = None) -> list[dict]:
        ...
        dense_results = self._vector_store.query(query_emb, top_k=self.recall_top_k, paper_id=paper_id)
        sparse_results = self._bm25.search(query, top_k=self.recall_top_k, paper_id=paper_id)
        ...
        ranked = sorted(merged.values(), key=lambda x: (-x["score"], x.get("chunk_id", "")))
        return ranked[:top_k]

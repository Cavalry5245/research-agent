...
# Apply WSL compatibility patch if needed
try:
    from zotero_mcp.wsl_patch import apply_wsl_patch
    apply_wsl_patch()
except ImportError:
    pass  # Patch module not available (older installations)
...
def get_zotero_client() -> zotero.Zotero:
    ...
    # WSL compatibility: Check for custom host
    custom_host = os.getenv("ZOTERO_LOCAL_HOST")
    if custom_host:
        import sys
        print(f"[WSL PATCH get_zotero_client] Using custom Zotero host: {custom_host}", file=sys.stderr, flush=True)
    ...
    # WSL compatibility: Replace localhost with custom host if specified
    if local and custom_host:
        import sys
        print(f"[WSL PATCH] Inspecting client attributes...", file=sys.stderr, flush=True)

        # List all non-private attributes that might contain URLs
        url_attrs = [a for a in dir(client) if not a.startswith('_') and any(x in a.lower() for x in ['url', 'endpoint', 'base', 'host', 'server'])]
        print(f"[WSL PATCH] URL-related attrs: {url_attrs}", file=sys.stderr, flush=True)

        for attr in url_attrs:
            try:
                value = getattr(client, attr)
                if isinstance(value, str):
                    print(f"[WSL PATCH]   {attr} = {value[:80]}", file=sys.stderr, flush=True)
                    if 'localhost' in value or '127.0.0.1' in value:
                        new_value = value.replace('localhost', custom_host).replace('127.0.0.1', custom_host)
                        setattr(client, attr, new_value)
                        print(f"[WSL PATCH]   -> PATCHED to {new_value[:80]}", file=sys.stderr, flush=True)
            except Exception as e:
                print(f"[WSL PATCH]   {attr} = ERROR: {e}", file=sys.stderr, flush=True)
...
def get_local_zotero_client() -> zotero.Zotero | None:
    ...
    try:
        ...
        # WSL compatibility: Replace localhost with custom host if specified
        if custom_host:
            for attr in ['endpoint', 'api_url', 'base_url', 'endpoint_base', 'library_base']:
                if hasattr(client, attr):
                    old_value = getattr(client, attr)
                    if isinstance(old_value, str) and ('localhost' in old_value or '127.0.0.1' in old_value):
                        new_value = old_value.replace('localhost', custom_host).replace('127.0.0.1', custom_host)
                        setattr(client, attr, new_value)
        ...
        import sys
        print(f"[WSL PATCH] ERROR in get_local_zotero_client: {e}", file=sys.stderr, flush=True)
        return None
